...
public class ValidarArquivos {
    ...
    public Path validarDirEntrada(String caminhoDirEntrada) {
        System.out.println("Validação do diretorio de entrada...");
        ...
        if (!Files.isDirectory(dirEntrada)) {
            mensagemValidacao(caminhoDirEntrada, "Caminho informado não é um diretorio");
            return null;
        }
        ...
        if (arquivos == null || arquivos.length == 0) {
            mensagemValidacao(caminhoDirEntrada, "não há arquivos no diretorio informado");
            return null;
        }

        lstArquivos = Arrays.asList(arquivos);

        System.out.println("Diretorio validado com sucesso " + caminhoDirEntrada);
        return dirEntrada;
    }
